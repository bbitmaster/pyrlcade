...
space = hp.choice('main_choice',[{
    'activation_function'       : hp.choice('a_function',['tanh','linear','linear_rectifier','squash','sigmoid']),
    'activation_function_final' : hp.choice('a_function_final',['tanh','squash','linear']),
    'num_hidden'                : hp.choice('n_hidden',[32,64,128,256,512,1024,2048]),
...
def objective(space):
    ...
    params['simname'] = 'pyrlcade_hyperopt_nn_nnettest'
...
trials = MongoTrials(mongodb_machine + 'pyrlcade_nn_tests2/jobs',exp_key='pyrlcade_nn_nnettest_bugfix1')
